...
The exchange rate used in the hash must be the open market exchange rate of Eni, with respect to the denomination matching `Denom`. For example, if `Denom` is `uusd` and the going exchange rate for Eni is 1 USD, then "1" must be used as the exchange rate, as `1 ueni = 1 uusd`.
...
```go
// Deprecated: normal prevote and vote will be deprecated after columbus-4
// MsgExchangeRateVote - struct for voting on the exchange rate of Eni denominated in various Eni assets.
// For example, if the validator believes that the effective exchange rate of Eni in USD is 10.39, that's
// what the exchange rate field would be, and if 1213.34 for KRW, same.
type MsgExchangeRateVote struct {
	ExchangeRate sdk.Dec        // the effective rate of Eni in {Denom}
	Salt         string
	Denom        string
	Feeder       sdk.AccAddress
	Validator    sdk.ValAddress
...
> Delegate validators will likely require you to deposit some funds (in Eni or Eni) which they can use to pay fees, sent in a separate MsgSend. This agreement is made off-chain and not enforced by the Eni protocol.
...
```go
// MsgAggregateExchangeRateVote - struct for voting on the exchange rates of Eni denominated in various Eni assets.
