...
func TransactionDependencyGenerator(_ aclkeeper.Keeper, evmKeeper evmkeeper.Keeper, ctx sdk.Context, msg sdk.Msg) ([]sdkacltypes.AccessOperation, error) {
	...
	sender := evmMsg.Derived.SenderEniAddr
	...
	toAddress := tx.To()
	if toAddress != nil {
		eniAddress := evmKeeper.GetEniAddressOrDefault(ctx, *toAddress)
		ops = appendRWBalanceOps(ops, eniAddress)
		ops = append(ops, sdkacltypes.AccessOperation{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_E2S,
			IdentifierTemplate: hex.EncodeToString(evmtypes.EVMAddressToEniAddressKey(*toAddress)),
		}, sdkacltypes.AccessOperation{
			AccessType:         sdkacltypes.AccessType_READ,
	...
	return append(ops, []sdkacltypes.AccessOperation{
		...
		},
		{
			...
			IdentifierTemplate: hex.EncodeToString(evmtypes.EniAddressToEVMAddressKey(evmKeeper.AccountKeeper().GetModuleAddress(authtypes.FeeCollectorName))),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_E2S,
			IdentifierTemplate: hex.EncodeToString(evmtypes.EVMAddressToEniAddressKey(evmAddr)),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_S2E,
			IdentifierTemplate: hex.EncodeToString(evmtypes.EniAddressToEVMAddressKey(evmMsg.Derived.SenderEniAddr)),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_CODE_HASH,
