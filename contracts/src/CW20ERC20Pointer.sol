...
contract CW20ERC20Pointer is ERC20 {
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        ...
        string memory ownerAddr = _formatPayload("address", _doubleQuotes(AddrPrecompile.getEniAddr(owner)));
    ...
    function allowance(address owner, address spender) public view override returns (uint256) {
        string memory o = _formatPayload("owner", _doubleQuotes(AddrPrecompile.getEniAddr(owner)));
        string memory s = _formatPayload("spender", _doubleQuotes(AddrPrecompile.getEniAddr(spender)));
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        ...
        if (currentAllowance > amount) {
            string memory spenderAddr = _formatPayload("spender", _doubleQuotes(AddrPrecompile.getEniAddr(spender)));
    ...
    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        require(to != address(0), "ERC20: transfer to the zero address");
        string memory sender = _formatPayload("owner", _doubleQuotes(AddrPrecompile.getEniAddr(from)));
        string memory recipient = _formatPayload("recipient", _doubleQuotes(AddrPrecompile.getEniAddr(to)));
        string memory amt = _formatPayload("amount", _doubleQuotes(Strings.toString(amount)));
