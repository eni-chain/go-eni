...
const { ABI, deployErc20PointerForCw20, deployErc721PointerForCw721, getEniAddress, deployWasm, execute, delay, isDocker } = require("../../contracts/test/lib.js");
...
async function deployCw20WithPointer(deployerEniAddr, signer, time, evmRpc="") {
  ...
  const cw20Address = await deployWasm(CW20_BASE_PATH, deployerEniAddr, "cw20", {
    name: `testCw20${time}`,
    symbol: "TEST",
    decimals: 6,
    initial_balances: [
      { address: deployerEniAddr, amount: hre.ethers.utils.parseEther("1000000").toString() }
    ],
    mint: {
      "minter": deployerEniAddr, "cap": hre.ethers.utils.parseEther("10000000").toString()
    }
  }, deployerEniAddr);
  const pointerAddr = await deployErc20PointerForCw20(hre.ethers.provider, cw20Address, 10, deployerEniAddr, evmRpc);
...
async function deployCw721WithPointer(deployerEniAddr, signer, time, evmRpc="") {
  ...
  const cw721Address = await deployWasm(CW721_BASE_PATH, deployerEniAddr, "cw721", {
    "name": `testCw721${time}`,
    "symbol": "TESTNFT",
    "minter": deployerEniAddr,
    "withdraw_address": deployerEniAddr,
  }, deployerEniAddr);
  const pointerAddr = await deployErc721PointerForCw721(hre.ethers.provider, cw721Address, deployerEniAddr, evmRpc);
...
async function doesTokenFactoryDenomExist(denom) {
  const output = await execute(`enid q tokenfactory denom-authority-metadata ${denom} --output json`);
  const parsed = JSON.parse(output);

  return parsed.authority_metadata.admin !== "";
}

async function sendFunds(amountEni, recipient, signer) {

  const bal = await signer.getBalance();
  if (bal.lt(hre.ethers.utils.parseEther(amountEni))) {
    throw new Error(`Signer has insufficient balance. Want ${hre.ethers.utils.parseEther(amountEni)}, has ${bal}`);
  }

  const gasLimit = await signer.estimateGas({
    to: recipient,
    value: hre.ethers.utils.parseEther(amountEni)
  })
...
const mintCw721 = async (contractAddress, address, id) => {
  ...
  const command = `enid tx wasm execute ${contractAddress} "${jsonString}" --from=${address} --gas=500000 --gas-prices=0.1ueni --broadcast-mode=block -y --output=json`;
...
const getValidators = async () => {
  const command = `enid q staking validators --output json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const getCodeIdFromContractAddress = async (contractAddress) => {
  const command = `enid q wasm contract ${contractAddress} --output json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const instantiateHubContract = async (
  ...
  const command = `enid tx wasm instantiate ${codeId} "${jsonString}" --label ${label} --admin ${adminAddress} --from ${adminAddress} --gas=5000000 --fees=1000000ueni -y --broadcast-mode block -o json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const bond = async (contractAddress, address, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `enid tx wasm execute ${contractAddress} "${jsonString}" --amount=${amount}ueni --from=${address} --gas=500000 --gas-prices=0.1ueni --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
...
const unbond = async (hubAddress, tokenAddress, address, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `enid tx wasm execute ${tokenAddress} "${jsonString}" --from=${address} --gas=500000 --gas-prices=0.1ueni --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
...
const queryTokenBalance = async (contractAddress, address) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `enid q wasm contract-state smart ${contractAddress} "${jsonString}" --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const addAccount = async (accountName) => {
  const command = `enid keys add ${accountName}-${Date.now()} --output=json --keyring-backend test`;
  const output = await execute(command);
...
const transferTokens = async (tokenAddress, sender, destination, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `enid tx wasm execute ${tokenAddress} "${jsonString}" --from=${sender} --gas=200000 --gas-prices=0.1ueni --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
...
async function setupAccountWithMnemonic(baseName, mnemonic, deployer) {
  ...
  const address = await getEniAddress(deployer.address)
...
async function addDeployerAccount(keyName, address, mnemonic) {
  ...
  try {
    const output = await execute(`enid keys show ${address} --output json --keyring-backend test`);
    return JSON.parse(output);
  } catch (e) {}
  ...
  try {
    ...
    if (await isDocker()) {
      ...
      output = await execute(`enid keys add ${keyName} --recover --keyring-backend test`,`printf "${mnemonic}"`)
    } else {
      output = await execute(`printf "${mnemonic}" | enid keys add ${keyName} --recover --keyring-backend test`)
    }
  }
  catch (e) {}

  // If both of the calls above fail, this one will fail.
  const output = await execute(`enid keys show ${keyName} --output json --keyring-backend test`);
  return JSON.parse(output);
}
