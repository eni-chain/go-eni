# Eni V2 Config Migration Guide

## Intro
EniV2 introduces a parallelized EVM along with a host of other features to the Eni blockchain
This doc is meant to help node operators with migrating their configs when running on EniV2.
...
mempool config. For the `app.toml`, we will make sure to enable enidb and add in a whole section for evm.
...
## Full Example EniV2 config.toml
...
This is just for validator nodes. Reference `enidb_migration.md` for more information on enabling state store for rpc nodes.
...
# Pruning strategy is completely ignored when enidb is enabled
pruning = "default"

# These are applied if and only if the pruning strategy is custom, and enidb is not enabled
