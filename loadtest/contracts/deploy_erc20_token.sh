...
INPUT=$(forge create -r "$evm_endpoint" --private-key 57acb95d82739866a5c29e40b0aa2590742ae50425b7dd5b5d279a986370189e src/ERC20Token.sol:ERC20Token --json --constructor-args "TEST_TOKEN" "TT" | jq -r '.transaction.input')

deploy_output=$(cast send --rpc-url $evm_endpoint --private-key 57acb95d82739866a5c29e40b0aa2590742ae50425b7dd5b5d279a986370189e --gas-limit 700000 --gas-price 1000000000 --create $INPUT --json | jq -r '.contractAddress')
