...
func TestEndOfEpochMintedCoinDistribution(t *testing.T) {
	...
	t.Run("Initial should be zero", func(t *testing.T) {
		eniApp := keepertest.TestApp()
		ctx := eniApp.BaseApp.NewContext(false, tmproto.Header{Time: time.Now()})

		header := tmproto.Header{
			Height: eniApp.LastBlockHeight() + 1,
			Time:   time.Now().UTC(),
		}
		eniApp.BeginBlock(ctx, abci.RequestBeginBlock{Header: header})
		require.Equal(t, int64(0), eniApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin().Amount.Int64())
	})
	...
	t.Run("even full release", func(t *testing.T) {
		...
		eniApp.MintKeeper.SetParams(ctx, mintParams)

		for i := 0; i < 25; i++ {
			currTime := genesisTime.AddDate(0, 0, i)
			currEpoch := getEpoch(genesisTime, currTime)
			eniApp.EpochKeeper.BeforeEpochStart(ctx, currEpoch)
			eniApp.EpochKeeper.AfterEpochEnd(ctx, currEpoch)
			mintParams = eniApp.MintKeeper.GetParams(ctx)

			// 250k / 25 days = 100000 per day
			expectedAmount := int64(100000)
			newMinter := eniApp.MintKeeper.GetMinter(ctx)
...
func TestNoEpochPassedNoDistribution(t *testing.T) {
	...
	header := tmproto.Header{Height: eniApp.LastBlockHeight() + 1}
	eniApp.BeginBlock(ctx, abci.RequestBeginBlock{Header: header})
	// Get mint params
	mintParams := eniApp.MintKeeper.GetParams(ctx)
	genesisTime := time.Date(2022, time.Month(7), 18, 10, 0, 0, 0, time.UTC)
	presupply := eniApp.BankKeeper.GetSupply(ctx, mintParams.MintDenom)
	startLastMintAmount := eniApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin()
	...
	for i := 0; i < 60*24*7*52-1; i++ {
		...
		currSupply := eniApp.BankKeeper.GetSupply(ctx, mintParams.MintDenom)
		require.True(t, currSupply.IsEqual(presupply))
	}
	// Ensure that EpochProvision hasn't changed
	endLastMintAmount := eniApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin()
