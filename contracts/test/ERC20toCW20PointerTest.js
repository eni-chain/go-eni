...
describe("ERC20 to CW20 Pointer", function () {
    ...
    before(async function () {
        ...
        cw20Address = await deployWasm(WASM.CW20, accounts[0].eniAddress, "cw20", {
            name: "Test",
            symbol: "TEST",
            decimals: 6,
            initial_balances: [
                { address: admin.eniAddress, amount: "1000000" },
                { address: accounts[0].eniAddress, amount: "2000000" },
                { address: accounts[1].eniAddress, amount: "3000000" }
            ],
            mint: {
                "minter": admin.eniAddress, "cap": "99900000000"
            }
        });
    });
        ...
        describe("pointer functions", function () {
            ...
            describe("transfer()", function () {
                it("should transfer", async function () {
                    ...
                    const eniBlock = await ethers.provider.send('eni_getBlockByNumber', ['0x' + blockNumber.toString(16), false]);
                    expect(ethBlock.transactions.length).to.equal(1);
                    expect(eniBlock.transactions.length).to.equal(1);

                    const ethReceipts = await ethers.provider.send('eth_getBlockReceipts', ['0x' + blockNumber.toString(16)]);
                    const eniReceipts = await ethers.provider.send('eni_getBlockReceipts', ['0x' + blockNumber.toString(16)]);
                    expect(ethReceipts.length).to.equal(1);
                    expect(eniReceipts.length).to.equal(1);
                    expect(ethReceipts[0].transactionHash).to.equal(eniReceipts[0].transactionHash);
