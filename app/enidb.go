...
import (
	...
	"cosmossdk.io/log"
	"cosmossdk.io/store/rootmultiv2"
...
) ([]func(*baseapp.BaseApp), types.StateStore) {
	scEnabled := cast.ToBool(appOpts.Get(FlagSCEnable))
	if !scEnabled {
		logger.Info("EniDB is disabled, falling back to IAVL")
		return baseAppOptions, nil
	}
	logger.Info("EniDB SC is enabled, running node with StoreV2 commit store")
	scConfig := parseSCConfigs(appOpts)
	ssConfig := parseSSConfigs(appOpts)
	if ssConfig.Enable {
		logger.Info(fmt.Sprintf("EniDB StateStore is enabled, running %s for historical state", ssConfig.Backend))
	}
	validateConfigs(appOpts)

	// cms must be overridden before the other options, because they may use the cms,
	// make sure the cms aren't be overridden by the other options later on.
	cms := rootmultiv2.NewStore(homePath, logger, scConfig, ssConfig, cast.ToBool(appOpts.Get("migrate-iavl")))
...
func validateConfigs(appOpts servertypes.AppOptions) {
	...
	if snapshotExportInterval > 0 && scEnabled {
		if !ssEnabled {
			panic(fmt.Sprintf("Config validation failed, EniDB SS store needs to be enabled when snapshot interval %d > 0", snapshotExportInterval))
		}
	}
}
