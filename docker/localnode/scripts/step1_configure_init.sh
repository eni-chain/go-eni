...
MONIKER="eni-node-$NODE_ID"

enid init "$MONIKER" --chain-id eni >/dev/null 2>&1
...
ENI_NODE_ID=$(enid tendermint show-node-id)
NODE_IP=$(hostname -i | awk '{print $1}')
echo "$ENI_NODE_ID@$NODE_IP:26656" >> build/generated/persistent_peers.txt
...
GENESIS_ACCOUNT_ADDRESS=$(printf "12345678\n" | enid keys show "$ACCOUNT_NAME" -a)
echo "$GENESIS_ACCOUNT_ADDRESS" >> build/generated/genesis_accounts.txt

# Add funds to genesis account
enid add-genesis-account "$GENESIS_ACCOUNT_ADDRESS" 10000000ueni,10000000uusdc,10000000uatom

# Create gentx
printf "12345678\n" | enid gentx "$ACCOUNT_NAME" 10000000ueni --chain-id eni
cp ~/.eni/config/gentx/* build/generated/gentx/
...
# Set node enivaloper info
ENIVALOPER_INFO=$(printf "12345678\n" | enid keys show "$ACCOUNT_NAME" --bech=val -a)
PRIV_KEY=$(printf "12345678\n12345678\n" | enid keys export "$ACCOUNT_NAME")
echo "$PRIV_KEY" >> build/generated/exported_keys/"$ENIVALOPER_INFO".txt
...
sed -i.bak -e "s|^validator *=.*|validator = \"$ENIVALOPER_INFO\"|" $ORACLE_CONFIG_FILE
