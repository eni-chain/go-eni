...
do
    $HOME/go/bin/iaviewer data $HOME/.eni/data/application.db "s/k:"$key"/" $HEIGHT > $HOME/state_$HEIGHT/$key.data
    $HOME/go/bin/iaviewer shape $HOME/.eni/data/application.db "s/k:"$key"/" $HEIGHT > $HOME/state_$HEIGHT/$key.shape
done
