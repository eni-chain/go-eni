...
type ExchangeRateVote struct {
	ExchangeRate sdk.Dec        // ExchangeRate of Eni in target fiat currency
...
An `sdk.Dec` that stores the current Eni exchange rate against a given denom, which is used by the [Market](../../market/spec/README.md) module for pricing swaps.

You can get the active list of denoms trading against `Eni` (denominations with votes past `VoteThreshold`) with `k.GetActiveDenoms()`.
...
type AggregateExchangeRateVote struct {
	ExchangeRateTuples ExchangeRateTuples // ExchangeRates of Eni in target fiat currencies
	Voter              sdk.ValAddress     // voter val address of validator
}
```
