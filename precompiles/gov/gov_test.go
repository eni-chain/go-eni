...
func TestGovPrecompile(t *testing.T) {
	...
	tests := []struct {
		name           string
		args           args
		setup          func(ctx sdk.Context, k *keeper.Keeper, evmAddr common.Address, eniAddr sdk.AccAddress)
		verify         func(t *testing.T, ctx sdk.Context, eniAddr sdk.AccAddress, proposalID uint64)
		wantErr        bool
		avoidAssociate bool
	}{
		{
		...
		},
		{
			...
			verify: func(t *testing.T, ctx sdk.Context, eniAddr sdk.AccAddress, proposalID uint64) {
				v, found := testApp.GovKeeper.GetVote(ctx, proposalID, eniAddr)
		...
		},
		{
			...
			setup: func(ctx sdk.Context, k *keeper.Keeper, evmAddr common.Address, eniAddr sdk.AccAddress) {},
			verify: func(t *testing.T, ctx sdk.Context, eniAddr sdk.AccAddress, proposalID uint64) {
				_, found := testApp.GovKeeper.GetVote(ctx, proposalID, eniAddr)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.setup(ctx, k, evmAddr, eniAddr)
			...
			if tt.wantErr {
				...
				tt.verify(t, ctx, eniAddr, tt.args.proposal)
			}
		})
	}
}
