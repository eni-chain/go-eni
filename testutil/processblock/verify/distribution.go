...
// assuming only `ueni` will get distributed
func Allocation(t *testing.T, app *processblock.App, f BlockRunnable, _ []signing.Tx) BlockRunnable {
	return func() []uint32 {
		...
		feesCollectedInt := app.BankKeeper.GetBalance(app.Ctx(), feeCollector.GetAddress(), "ueni")
...
func getOutstandingRewards(app *processblock.App) map[string]sdk.DecCoin {
	...
	for _, val := range app.GetAllValidators() {
		outstandingRewards[val.GetOperator().String()] = sdk.NewDecCoin("ueni", sdk.NewInt(0))
	}
	app.DistrKeeper.IterateValidatorOutstandingRewards(
		...
		func(val sdk.ValAddress, rewards types.ValidatorOutstandingRewards) (stop bool) {
			...
			if len(rewards.Rewards) > 1 {
				panic("expecting only ueni as validator reward denom but found multiple")
			}
			if rewards.Rewards[0].Denom != "ueni" {
				panic(fmt.Sprintf("expecting only ueni as validator reward denom but found %s", rewards.Rewards[0].Denom))
			}
