...
func TestPrecompile_Run_Delegation(t *testing.T) {
	...
	validatorAddress := "enivaloper134ykhqrkyda72uq7f463ne77e4tn99steprmz7"
	...
	delegationResponse := &stakingtypes.QueryDelegationResponse{
		DelegationResponse: &stakingtypes.DelegationResponse{
			Delegation: stakingtypes.Delegation{
				DelegatorAddress: callerEniAddress.String(),
				ValidatorAddress: validatorAddress,
				Shares:           sdk.NewDec(int64(shares)),
			},
			Balance: sdk.NewCoin("ueni", sdk.NewInt(int64(shares))),
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				stakingQuerier: &TestStakingQuerier{
					Err: fmt.Errorf("delegation with delegator %s not found for validator", callerEniAddress.String()),
				},
			},
			args: args{
				delegatorAddress: callerEvmAddress,
			...
			wantErr:    true,
			wantErrMsg: fmt.Sprintf("delegation with delegator %s not found for validator", callerEniAddress.String()),
		},
		{
