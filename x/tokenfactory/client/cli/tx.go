...
const (
	...
	FlagAllowListDescription = "Path to the allow list JSON file with an array of addresses " +
		...
		"[\"addr1\", \"addr2\"]}, where addr1 and addr2 are bech32 Eni native addresses or EVM addresses."
)
...
func NewSetDenomMetadataCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(
			`
Example:
$ enid tx tokenfactory set-denom-metadata <path/to/metadata.json> --from=<key_or_address>
...
func ParseAllowListJSON(allowListFile string, queryClient evmtypes.QueryClient) (banktypes.AllowList, error) {
	...
	for _, addr := range allowList.Addresses {
		...
		if common.IsHexAddress(addr) {
			res, err := queryClient.EniAddressByEVMAddress(context.Background(), &evmtypes.QueryEniAddressByEVMAddressRequest{EvmAddress: addr})
			...
			uniqueAddresses = append(uniqueAddresses, res.EniAddress)
			continue
		}
