...
type Config struct {
	...
	TLS                bool                  `json:"tls"`
	EniTesterAddress   string                `json:"eni_tester_address"`
...
type NumericDistribution struct {
	Min         math.Dec `json:"min"`
	Max         math.Dec `json:"max"`
	NumDistinct int64    `json:"number_of_distinct_values"`
}

// todo: add more distributions
func (d *NumericDistribution) Sample() math.Dec {
	//steps := math.NewDecFromInt64(rand.Int63n(d.NumDistinct))
	//return d.Min.Add(d.Max.Sub(d.Min).QuoInt64(d.NumDistinct).Mul(steps))
	return math.Dec{}
}

// Invalid numeric distribution sample
func (d *NumericDistribution) InvalidSample() math.Dec {
	//steps := math.NewDecFromInt64(rand.Int63n(d.NumDistinct))
	//if rand.Float64() < 0.5 {
	//	return d.Min.Add(d.Max.Sub(d.Min).QuoInt64(d.NumDistinct).Mul(steps))
	//}
...
type DexMsgTypeDistribution struct {
	LimitOrderPct  math.Dec `json:"limit_order_percentage"`
	MarketOrderPct math.Dec `json:"market_order_percentage"`
}

type StakingMsgTypeDistribution struct {
	DelegatePct        math.Dec `json:"delegate_percentage"`
	UndelegatePct      math.Dec `json:"undelegate_percentage"`
	BeginRedelegatePct math.Dec `json:"begin_redelegate_percentage"`
}
...
//	if !d.Dex.LimitOrderPct.Add(d.Dex.MarketOrderPct).Equal(math.OneDec()) {
...
//		panic("Distribution percentages must add up to 1")
//	}
//	randNum, err := math.NewDecFromString(fmt.Sprintf("%f", rand.Float64()))
...
//	if !utils.Reduce(*d, func(i ContractDistribution, o math.Dec) math.Dec { return o.Add(i.Percentage) }, math.ZeroInt()).Equal(math.OneDec()) {
//		panic("Distribution percentages must add up to 1")
//	}
//	randNum := sdk.MustNewDecFromStr(fmt.Sprintf("%f", rand.Float64()))
//	cumPct := sdk.ZeroDec()
...
type ContractDistribution struct {
	ContractAddr string   `json:"contract_address"`
	Percentage   math.Dec `json:"percentage"`
}
