...
printf "12345678\n12345678\ny\n" | enid keys add $ACCOUNT_NAME >/dev/null 2>&1

override_genesis() {
  cat ~/.eni/config/genesis.json | jq "$1" > ~/.eni/config/tmp_genesis.json && mv ~/.eni/config/tmp_genesis.json ~/.eni/config/genesis.json;
}

override_genesis '.app_state["crisis"]["constant_fee"]["denom"]="ueni"'
override_genesis '.app_state["mint"]["params"]["mint_denom"]="ueni"'
override_genesis '.app_state["staking"]["params"]["bond_denom"]="ueni"'
...
# gov parameters
override_genesis '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="ueni"'
override_genesis '.app_state["gov"]["deposit_params"]["min_expedited_deposit"][0]["denom"]="ueni"'
...
while read account; do
  echo "Adding: $account"
  enid add-genesis-account "$account" 1000000000000000000000ueni,1000000000000000000000uusdc,1000000000000000000000uatom
done <build/generated/genesis_accounts.txt

# add funds to admin account
printf "12345678\n" | enid add-genesis-account admin 1000000000000000000000ueni,1000000000000000000000uusdc,1000000000000000000000uatom

mkdir -p ~/exported_keys
cp -r build/generated/gentx/* ~/.eni/config/gentx
...
enid collect-gentxs >/dev/null 2>&1

cp ~/.eni/config/genesis.json build/generated/genesis.json
