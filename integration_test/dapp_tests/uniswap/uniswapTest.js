...
const { fundAddress, createTokenFactoryTokenAndMint, deployErc20PointerNative, execute, getEniAddress, queryWasm, getEniBalance, isDocker, ABI } = require("../../../contracts/test/lib.js");
...
describe("Uniswap Test", function () {
    ...
    before(async function () {
        ...
        tokenFactoryDenom = await createTokenFactoryTokenAndMint(tokenName, hre.ethers.utils.parseEther("1000000").toString(), deployerEniAddr, deployerEniAddr)
        console.log("DENOM", tokenFactoryDenom)
        const pointerAddr = await deployErc20PointerNative(hre.ethers.provider, tokenFactoryDenom, deployerEniAddr, evmRpcUrls[testChain])
        ...
        const cw20Details = await deployCw20WithPointer(deployerEniAddr, deployer, time, evmRpcUrls[testChain])
        ...
        // Gets the amount of WETH9 required to instantiate pools by depositing Eni to the contract
    ...
    describe("Swaps", function () {
        ...
        it("Associated account should swap erc20-tokenfactory successfully", async function () {
            ...
            const userBal = await getEniBalance(userEniAddr, tokenFactoryDenom)
        ...
        it("Unassociated account should receive erc20-tokenfactory tokens successfully", async function () {
            ...
            const userEniAddr = await getEniAddress(unassocUser.address);
