...
The association between the eni address and the eth address will also be created here if using keyring-backend test.
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if keyringBackend == keyring.BackendTest {
				// associate the eth address with the eni address through the genesis file
				evmGenState := evm.GetGenesisStateFromAppState(depCdc, appState)
				eniEthAddrAssociation := evmtypes.AddressAssociation{
					EniAddress: addr.String(),
					EthAddress: ethAddr.Hex(),
				}
				evmGenState.AddressAssociations = append(evmGenState.AddressAssociations, &eniEthAddrAssociation)
...
func getPrivateKeyOfAddr(kb keyring.Keyring, addr sdk.Address) (*ecdsa.PrivateKey, error) {
	...
	for _, key := range keys {
		...
		if localInfo.GetAddress().Equals(addr) {
			...
			// Need to use private key to convert to eni address here
