...
func TestQueryExchangeRates(t *testing.T) {
	...
	rate := sdk.NewDec(1700)
	input.OracleKeeper.SetBaseExchangeRate(input.Ctx, utils.MicroAtomDenom, rate)
	input.OracleKeeper.SetBaseExchangeRate(input.Ctx, utils.MicroEniDenom, rate)
	...
	require.Equal(t, types.DenomOracleExchangeRatePairs{
		...
		types.NewDenomOracleExchangeRatePair(utils.MicroEniDenom, rate, sdk.ZeroInt(), input.Ctx.BlockTime().UnixMilli()),
