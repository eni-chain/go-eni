...
describe("CW20 to ERC20 Pointer", function () {
        ...
        describe("pointer functions", function () {
            ...
            describe("query", function(){
                ...
                it("should return balance", async function(){
                    const result = await queryWasm(pointer, "balance", {address: accounts[0].eniAddress});
                    expect(result).to.deep.equal({ data: { balance: balances[0].toString() } });
                });

                it("should return allowance", async function(){
                    const result = await queryWasm(pointer, "allowance", {owner: accounts[0].eniAddress, spender: accounts[0].eniAddress});
                ...
                it("should throw exception on unsupported endpoints", async function() {
                    ...
                    await assertUnsupported(pointer, "all_allowances", { owner: accounts[0].eniAddress });
            ...
            describe("execute", function() {
                it("should transfer token", async function() {
                    const respBefore = await queryWasm(pointer, "balance", {address: accounts[1].eniAddress});
                    const balanceBefore = respBefore.data.balance;

                    const res = await executeWasm(pointer,  { transfer: { recipient: accounts[1].eniAddress, amount: "100" } });
                    ...
                    expect(enilogs[0]["topics"][0]).to.equal(ethers.id("Transfer(address,address,uint256)"));
                    const respAfter = await queryWasm(pointer, "balance", {address: accounts[1].eniAddress});
                ...
                it("transfer to unassociated address should fail", async function() {
                    const unassociatedEniAddr = "eni1z7qugn2xy4ww0c9nsccftxw592n4xhxccmcf4q";
                    const respBefore = await queryWasm(pointer, "balance", {address: accounts[1].eniAddress});
                    const balanceBefore = respBefore.data.balance;

                    await executeWasm(pointer,  { transfer: { recipient: unassociatedEniAddr, amount: "100" } });
                    const respAfter = await queryWasm(pointer, "balance", {address: accounts[1].eniAddress});
                    const balanceAfter = respAfter.data.balance;
                ...
                it("transfer to contract address should succeed", async function() {
                    ...
                    const respBefore = await queryWasm(pointer, "balance", {address: admin.eniAddress});
                    const balanceBefore = respBefore.data.balance;

                    await executeWasm(pointer,  { transfer: { recipient: pointer, amount: "100" } });
                    const respAfter = await queryWasm(pointer, "balance", {address: admin.eniAddress});
                    const balanceAfter = respAfter.data.balance;
                ...
                it("should increase and decrease allowance for a spender", async function() {
                    const spender = accounts[0].eniAddress;
                    await executeWasm(pointer, { increase_allowance: { spender: spender, amount: "300" } });

                    let allowance = await queryWasm(pointer, "allowance", { owner: admin.eniAddress, spender: spender });
                    expect(allowance.data.allowance).to.equal("300");

                    await executeWasm(pointer, { decrease_allowance: { spender: spender, amount: "300" } });

                    allowance = await queryWasm(pointer, "allowance", { owner: admin.eniAddress, spender: spender });
                ...
                it("should transfer token using transferFrom", async function() {
                    ...
                    await resp.wait();
                    const respBefore = await queryWasm(pointer, "balance", {address: accounts[0].eniAddress});
                    const balanceBefore = respBefore.data.balance;
                    await executeWasm(pointer,  { transfer_from: { owner: accounts[0].eniAddress, recipient: accounts[1].eniAddress, amount: "100" } });
                    const respAfter = await queryWasm(pointer, "balance", {address: accounts[0].eniAddress});
                    const balanceAfter = respAfter.data.balance;
                    expect(balanceAfter).to.equal((parseInt(balanceBefore) - 100).toString());
                });
                ...
                it("should transfer if called through wasmd precompile", async function() {
                    ...
                    const transferMsg = { transfer: { recipient: accounts[1].eniAddress, amount: "100" } };
    ...
    describe("Pointer Functionality", function () {
        ...
        // Pointer version is going to be coupled with enid version going forward (as in,
        // given a enid version, it's impossible to have multiple versions of pointer).
        ...
        describe.skip("Pointer Upgrade", function () {
            ...
            before(async function () {
               const tokenAddr = await testToken.getAddress();
               newPointer = await deployWasm(WASM.POINTER_CW20, accounts[0].eniAddress, "cw20-erc20", {erc20_address: tokenAddr })
