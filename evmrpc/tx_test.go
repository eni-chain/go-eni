...
//	for i, body := range []string{bodyByBlockNumberAndIndex, bodyByBlockHashAndIndex, bodyByHash} {
//		t.Run(fmt.Sprintf("TestCase%d", i), func(t *testing.T) {
//			testGetTransaction(t, body)
//		})
//	}
//}
//
//func testGetTransaction(t *testing.T, body string) {
//	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("http://%s:%d", TestAddr, TestPort), strings.NewReader(body))
//	require.Nil(t, err)
//	req.Header.Set("Content-Type", "application/json")
//	res, err := http.DefaultClient.Do(req)
...
//	body := fmt.Sprintf("{\"jsonrpc\": \"2.0\",\"method\": \"%s_getTransactionReceiptExcludeTraceFail\",\"params\":[\"%s\"],\"id\":\"test\"}", "eni", TestPanicTxHash)
//	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("http://%s:%d", TestAddr, TestPort), strings.NewReader(body))
//	require.Nil(t, err)
//	req.Header.Set("Content-Type", "application/json")
//	res, err := http.DefaultClient.Do(req)
